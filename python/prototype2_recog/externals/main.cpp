...
const DWORD dw_Rot = 0x26C;//client    
						   //engine.dll+4632D4 if wanna write or read angle from engine.dll
...
/*	
my health					"server.dll"+004F2FEC	offset	E4
first ct player health		"server.dll"+004F2FFC	offset	E4
second ct player			"server.dll"+004F300C	offset	E4

my team						"server.dll"+004F2FEC	offset 1F4



conclusion:
player0 = "server.dll"+004F2FEC
playerOffset = 10
healthOffset = E4
teamOffset   = 1F4
posOffset    = 280
...
const DWORD EntityList2 = 0x4F2FEC;	//server.dll
const DWORD EntityLoopDistance2 = 0x10;
const DWORD dw_mTeamOffset2 = 0x1F4;//client
const DWORD dw_Health2 = 0xE4;//client
const DWORD dw_Pos2 = 0x280;//client
...
struct PlayerList_t
{
	...
	void ReadInformation(int Player)
	{
		...
		ReadProcessMemory(fProcess.__HandleProcess, (PBYTE*)(fProcess.__dwordServer + EntityList2 + (Player * EntityLoopDistance2)), &CBaseEntity, sizeof(DWORD), 0);
		// Reading out our Team to our "Team" Varible.
		//ReadProcessMemory(fProcess.__HandleProcess, (PBYTE*)(CBaseEntity + dw_mTeamOffset), &Team, sizeof(int), 0);
		ReadProcessMemory(fProcess.__HandleProcess, (PBYTE*)(CBaseEntity + dw_mTeamOffset2), &Team, sizeof(int), 0);
		// Reading out our Health to our "Health" Varible.    
		//ReadProcessMemory(fProcess.__HandleProcess, (PBYTE*)(CBaseEntity + dw_Health), &Health, sizeof(int), 0);
		ReadProcessMemory(fProcess.__HandleProcess, (PBYTE*)(CBaseEntity + dw_Health2), &Health, sizeof(int), 0);
		// Reading out our Position to our "Position" Varible.
		//ReadProcessMemory(fProcess.__HandleProcess, (PBYTE*)(CBaseEntity + dw_Pos), &Position, sizeof(float[3]), 0);
		ReadProcessMemory(fProcess.__HandleProcess, (PBYTE*)(CBaseEntity + dw_Pos2), &Position, sizeof(float[3]), 0);
	}
}PlayerList[32];
...
void DisplayPlayersInfo() {
	...
	{
		//PlayerList[a].ReadInformation(a);
...
int main()
{
	...
	while (true)
	{
		...
		while (socket.receive(tmp, 8, received) != sf::Socket::Done) {
			cout << tmp << " -- " << received << endl;
			cout << "waiting for request" << endl;
			Sleep(1);
		}
		cout << tmp << "___________"<<received << endl;
		socket.setBlocking(true);*/


		//socket.receive(tmp, 8, received);
		//cout << tmp << "___________" << received << endl;
		...
		for (int a = 0; a < NumOfPlayers; a++)
		{
			//if (PlayerList[a].AimbotAngle[1] > 180.0f)
			//	PlayerList[a].AimbotAngle[1] = 360.0f - PlayerList[a].AimbotAngle[1];
			//else if (PlayerList[a].AimbotAngle[1] < -180.0f)
			//	PlayerList[a].AimbotAngle[1] = 360.0f + PlayerList[a].AimbotAngle[1];
			if (PlayerList[a].AimbotAngle[1] < 0.0f)
				PlayerList[a].AimbotAngle[1] = 360.0f + PlayerList[a].AimbotAngle[1];
