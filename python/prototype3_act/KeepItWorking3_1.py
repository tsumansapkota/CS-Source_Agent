...
def coord_to_pixel_fy_dust(xcoord, ycoord):
    centrex = int((xcoord - 16) / 3.113871635610766 + 42)
    centrey = int((ycoord + 1648) / 3.055555555555556 + 42)
    return centrex, centrey


def coord_to_pixel_de_dust2(xcoord, ycoord):
    centrex = int((xcoord + 896) / 7.55 + 21)
    centrey = int((ycoord + 2191.96875) / 7.55 + 10)
    return centrex, centrey
...
fps = 0
last_time = 0
mapImage = cv2.imread('../fy_dust.jpg')
print(np.size(mapImage))
# mapImage = cv2.resize(mapImage,None,fx=0.5,fy=0.5,interpolation=cv2.INTER_CUBIC)
    ...
    while True:
        ...
        try:
            ...
            if len(playerBoxes) > 0:
                ...
                if  movey != 12345 and movex != 12345 :#and win32api.GetAsyncKeyState(ord('F')) != 0
                    movex = int(movex * 6)
                    movey = int(movey * 6)
                    mouse.move(movex, movey)

                    if movex < 6 and movey < 3:
        ...
        for player, index in zip(players, range(len(players))):
            centrex, centrey = coord_to_pixel_fy_dust(player.position[1], player.position[0])
            if index == 0:
            ...
            # if player.position[1] > maxx:
            #     maxx = player.position[1]
            # elif player.position[1]<minx:
            #     minx = player.position[1]
            # if player.position[0] >maxy:
            #     maxy = player.position[0]
            # elif player.position[0]<miny:
            #     miny = player.position[0]
